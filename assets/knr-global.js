...
class SliderComponent extends HTMLElement {
  ...
  initPages() {
    ...
    this.scrollbarTrack = this.querySelector('.slider-scrollbar-track');
    this.updateScrollbar();

    this.update();
  }
    // funciton to update the scroll bar
    updateScrollbar() {
    if (!this.scrollbarThumb || !this.scrollbarTrack) return;

    const thumbWidthPercent = (this.slidesPerPage / this.sliderItemsToShow.length) * 100;
    const maxScrollLeft = this.slider.scrollWidth - this.slider.clientWidth;
    const scrollProgress = maxScrollLeft > 0 ? this.slider.scrollLeft / maxScrollLeft : 0;
    const maxThumbOffset = 100 - thumbWidthPercent;

    this.scrollbarThumb.style.width = `${thumbWidthPercent}%`;
    this.scrollbarThumb.style.left = `${scrollProgress * maxThumbOffset}%`;
  }
  ...
  update() {
    ...
    // custom scroll bar invoked on every scroll
    this.updateScrollbar();
